...
(function () {
  ...
  function getDefaultMealType() {
    ...
    if (h >= 5 && h < 10) return 'breakfast';  // 5-10 早餐
    if (h >= 10 && h < 15) return 'lunch';     // 10-15 午餐
    if (h >= 15 && h < 21) return 'dinner';    // 15-21 晚餐
    return 'snack';                            // 其他：加餐
  }
  ...
  function render(){
    var today=Y.today();
    var t=document.getElementById('dText'); if(t) t.textContent=cur===today?'今天':cur===Y.addDays(today,-1)?'昨天':(Number(cur.slice(5,7)))+'月'+(Number(cur.slice(8,10)))+'日';
    ...
    var r=document.getElementById('remain'); if(r){var diff=Math.round(goal-s.kcal);r.textContent=diff>=0?'还可摄入 '+diff+' kcal':'已超 '+(-diff)+' kcal';}
    if(window.YDJK_CHARTS){var ring=document.getElementById('kcalRing'); if(ring) YDJK_CHARTS.donutChart(ring,{value:s.kcal,max:goal,unit:'',label:'',size:88,decimals:0,color:s.kcal>goal?'#FF5D5D':undefined});}
    var mb=document.getElementById('macroBox'); if(mb){
      ...
      mb.innerHTML=items.map(function(it){var pct=Math.min(100,Math.round(it.v/it.t*100));return '<div class="yk-macro-bar"><small>'+it.n+' <b>'+it.v+'/'+it.t+'g</b></small><b>'+pct+'%</b><div class="yk-macro-track"><span style="width:'+pct+'%;background:'+it.c+'"></span></div></div>';}).join('');
    }

    var list=document.getElementById('mealList');
    var meals=Y.getMeals(cur);
    if(!meals.length){list.innerHTML='<div class="yk-empty" style="padding:20px"><div class="yk-empty-icon">🍽️</div><div class="yk-empty-title">还没有记录</div><div class="yk-empty-desc">点「记一餐」开始</div></div>';}
    else{
      ...
      /* 错误插入的 renderRecent，注释掉 */
      /*

  // 最近使用食物
  function renderRecent() {
    var el = document.getElementById('recentBox');
    if (!el) return;
    var recent = [];
    try { recent = JSON.parse(localStorage.getItem('ydjk:recent-foods') || '[]'); } catch(e) {}
    if (!recent.length) { el.innerHTML = ''; return; }
    el.innerHTML = '<div class="yk-card" style="padding:12px 14px"><div class="yk-text-xs yk-text-muted" style="font-weight:700;margin-bottom:6px">🕐 最近使用</div>' +
      '<div class="yk-flex" style="flex-wrap:wrap;gap:6px">' + recent.slice(0, 6).map(function(r) {
        return '<button class="js-recent-food" data-n="'+esc(r.name)+'" data-k="'+r.kcal+'" data-p="'+r.p+'" data-c="'+r.c+'" data-f="'+r.f+'" style="border:none;background:var(--bg-soft);color:var(--text-2);padding:6px 12px;border-radius:99px;font-size:.72rem;font-weight:700;cursor:pointer">'+esc(r.name)+'</button>';
      }).join('') + '</div></div>';
    el.querySelectorAll('.js-recent-food').forEach(function(b){
      b.onclick = function(){
      ...
      */
      ...
      list.querySelectorAll('.js-del').forEach(function(b){b.onclick=function(){Y.removeMeal(cur,b.dataset.id);render();};});
    }
    renderTpls();
  }
  ...
  function renderTpls(){
    ...
    el.querySelectorAll('.js-tpl').forEach(function(b){b.onclick=function(){
      var tpl=Y.getMealTemplates().filter(function(t){return t.id===b.dataset.id;})[0];
      if(!tpl) return;
      var type=preset||'breakfast';
  ...
  function openManual(){
    ...
    YK.openModal('<div class="yk-modal-title">✏️ 手动添加</div>'+
      '<div class="yk-field"><div class="yk-flex yk-gap-sm" id="mTypes">'+['breakfast','lunch','dinner','snack'].map(function(t,i){var active=t===manualType?' active':'';return '<button class="yk-type-btn'+active+'" data-t="'+t+'">'+types[t]+'</button>';}).join('')+'</div></div>'+
  ...
  function openMeal(f){
    ...
    YK.openModal('<div class="yk-modal-title">🍽️ '+esc(f.name)+'</div><div class="yk-field"><div class="yk-flex yk-gap-sm" id="types">'+['breakfast','lunch','dinner','snack'].map(function(t,i){return '<button class="yk-type-btn'+(i===0?' active':'')+'" data-t="'+t+'">'+types[t]+'</button>';}).join('')+'</div></div><div class="yk-field"><div class="yk-flex yk-gap"><input type="range" id="gram" min="10" max="500" value="100" step="10" style="flex:1"><span id="gramVal" style="min-width:48px;text-align:center;font-weight:800;font-size:.8rem;background:var(--bg-blue-soft);color:var(--blue);padding:5px 8px;border-radius:8px">100g</span></div></div><div id="calc" class="yk-text-sm yk-text-2" style="margin-bottom:6px"></div><div class="yk-modal-actions"><button class="yk-btn yk-btn-ghost" id="mMealNo">取消</button><button class="yk-btn yk-btn-primary" id="mMealYes">保存</button></div>');
      /* 修复：以下代码是错误插入，注释掉 */
      /*

      // 根据当前时间设置默认餐次高亮
      mask.querySelectorAll('#types .yk-type-btn').forEach(function(b){
      var type = defType; // 默认当前时间对应餐次
      ...
      */
        ...
        b.classList.toggle('active', b.dataset.t === defType);
      var type = defType; // 覆盖默认，使用当前时间对应餐次
    ...
    var type=preset||'breakfast';
      ...
      if (!preset) { type = defType; } // 无预设时使用当前时间餐次
