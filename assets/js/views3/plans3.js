...
(function () {
  ...
  function renderList(){
    ...
    list.innerHTML=wks.map(function(w){
      var met=Number(w.met)||5,mins=w.sets?w.sets*3:20,kcal=Math.round(met*3.5*60/200*mins);
  ...
  function renderPlans(){
    ...
    el.querySelectorAll('.js-ap').forEach(function(b){b.onclick=function(){
      var p=Y.getWorkoutPlans().filter(function(x){return x.id===b.dataset.i;})[0];
      if(!p||!DATA||!DATA.ACTIONS)return;
      var n=0;p.actions.forEach(function(a){var f=DATA.ACTIONS.filter(function(x){return x.name===a.action;})[0];if(f){Y.addWorkout(cur,{muscle:f.muscle,action:f.name,sets:a.sets||3,reps:a.reps||10,weight:null,minutes:null,met:f.muscle==='cardio'?7:5,distance:null,pace:null});n++;}});
      openAddWithPlan(p);
    };});
  }
  ...
  function openAdd(){
    ...
    function drawSel(){
      ...
      detail.innerHTML=ids.map(function(id){
        ...
        var isCardio = a && (a.muscle === 'cardio' || a.name.indexOf('跑')>=0 || a.name.indexOf('走')>=0 || a.name.indexOf('跳')>=0 || a.name.indexOf('游')>=0);
        if (isCardio) {
          // 有氧：分钟数
          return '<div class="yk-flex-between" style="padding:8px 10px;background:var(--bg-soft);border-radius:10px;margin-bottom:6px">'+
            ...
            '<span style="font-size:.7rem;color:var(--text-3);margin-left:2px">分钟</span>'+
          '</div>';
        }
        return '<div class="yk-flex-between" style="padding:8px 10px;background:var(--bg-soft);border-radius:10px;margin-bottom:6px">'+
          '<span style="font-size:.78rem;font-weight:700;flex:1">'+(a?esc(a.name):id)+'</span>'+
          ...
          '<input type="number" class="yk-input js-reps" data-i="'+id+'" value="'+s.reps+'" style="width:48px;padding:5px;font-size:.75rem;text-align:center" placeholder="次">'+
        '</div>';
      }).join('');
  ...
  // 套用训练计划：打开添加弹窗并预填动作
  function openAddWithPlan(plan) {
    if(!plan || !plan.actions || !plan.actions.length){YK.toast('计划数据错误','err');return;}
    openAddPlan(plan);
  }

  // 带预填的添加训练
  function openAddPlan(plan) {
    if(!DATA||!DATA.ACTIONS){YK.toast('动作库错误','err');return;}
    var sel={},muscles=[{id:'chest',label:'🏋️ 胸'},{id:'back',label:'🧗 背'},{id:'legs',label:'🦵 腿'},{id:'shoulder',label:'🏔️ 肩'},{id:'arms',label:'💪 手臂'},{id:'core',label:'🎯 核心'},{id:'cardio',label:'🏃 有氧'},{id:'yoga',label:'🧘 瑜伽'}],firstAction=plan.actions[0]||{},curM=firstAction.muscle||'chest';
    // 预填所选动作
    plan.actions.forEach(function(a){
      var f=DATA.ACTIONS.filter(function(x){return x.name===a.action;})[0];
      if(f)sel[f.id]={sets:a.sets||3,reps:a.reps||10};
    });
    YK.openModal('<div class="yk-modal-title">🏋️ '+esc(plan.name)+'</div><div class="yk-field"><input class="yk-input" type="date" id="wkDate" value="'+cur+'"></div><div class="yk-field"><div id="ms" class="yk-flex" style="flex-wrap:wrap;gap:6px"></div></div><div class="yk-field"><div id="acs" class="yk-flex" style="flex-wrap:wrap;gap:6px;max-height:22vh;overflow-y:auto"></div></div><div id="selDetail" style="margin-bottom:8px"></div><div id="selInfo" class="yk-text-xs yk-text-muted"></div><div class="yk-modal-actions"><button class="yk-btn yk-btn-ghost" id="aNo">取消</button><button class="yk-btn yk-btn-primary" id="aYes">保存训练</button></div>');
    var mask=document.querySelector('.yk-modal-mask.show');if(!mask)return;
    var ms=mask.querySelector('#ms'),acs=mask.querySelector('#acs'),info=mask.querySelector('#selInfo'),detail=mask.querySelector('#selDetail');
    function drawM(){ms.innerHTML=muscles.map(function(m){return '<button class="js-m" data-i="'+m.id+'" style="padding:7px 14px;border-radius:99px;border:1.5px solid '+(m.id===curM?'var(--blue)':'var(--line)')+';background:'+(m.id===curM?'var(--bg-blue-soft)':'#fff')+';color:'+(m.id===curM?'var(--blue)':'var(--text-2)')+';font-size:.72rem;font-weight:700;cursor:pointer">'+m.label+'</button>';}).join('');}
    function drawA(){var a=DATA.ACTIONS.filter(function(x){return x.muscle===curM;});if(!a.length)a=[{id:'c',name:'自定义'}];acs.innerHTML=a.map(function(x){var on=!!sel[x.id];return '<button class="js-a" data-i="'+x.id+'" style="padding:8px 14px;border-radius:99px;border:1.5px solid '+(on?'var(--blue)':'var(--line)')+';background:'+(on?'var(--bg-blue-soft)':'#fff')+';color:'+(on?'var(--blue)':'var(--text-2)')+';font-size:.72rem;font-weight:700;cursor:pointer;margin-bottom:4px">'+esc(x.name)+'</button>';}).join('');}
    function drawSel(){
      var ids=Object.keys(sel);
      if(!ids.length){info.textContent='请选择动作';detail.innerHTML='';return;}
      info.textContent='已选 '+ids.length+' 项 · 可调整组数/次数';
      detail.innerHTML=ids.map(function(id){
        var a=DATA.ACTIONS.find(function(x){return x.id===id;});
        var s=sel[id]||{sets:3,reps:10};
        return '<div class="yk-flex-between" style="padding:8px 10px;background:var(--bg-soft);border-radius:10px;margin-bottom:6px">'+
    ...
    mask.querySelector('#aYes').onclick=function(){
      ...
      ids.forEach(function(id){
        ...
        var s=sel[id]||{sets:3,reps:10};
        var isCardio = a && (a.muscle === 'cardio' || a.name.indexOf('跑')>=0 || a.name.indexOf('走')>=0 || a.name.indexOf('跳')>=0 || a.name.indexOf('游')>=0);
        Y.addWorkout(date,{muscle:a?a.muscle:'chest',action:a?a.name:'训练',sets:isCardio?1:(s.sets||3),reps:isCardio?1:(s.reps||10),weight:null,minutes:isCardio?(s.minutes||30):null,met:a?(a.muscle==='cardio'?7:5):5,distance:null,pace:null});
      });
      YK.closeModal(mask);YK.toast('✅ 已保存训练');cur=date;render();
    };
  }
